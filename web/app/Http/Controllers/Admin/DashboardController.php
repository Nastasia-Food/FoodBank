...
class DashboardController extends BackendController
{
    ...
    public function dayWiseOrderStatistics(Request $request)
    {
        ...
        if (auth()->user()->myrole == UserRole::ADMIN) {
            ...
            $order_statistics['outForDelivery'] = Order::whereBetween('created_at', [$startDate, $endDate])->where('order_type', OrderTypeStatus::DELIVERY)->where('status', OrderStatus::ON_THE_WAY)->count();
            ...
            $order_statistics['outForDelivery'] = Order::where('restaurant_id', $restaurantId)->whereBetween('created_at', [$startDate, $endDate])->where('order_type', OrderTypeStatus::DELIVERY)->where('status', OrderStatus::ON_THE_WAY)->count();
